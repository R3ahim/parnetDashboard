...
function CompnayShow3() {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch("https://deltaserverer.onrender.com/company3"); // Assumes time.json is in the public folder
    ...
    const handleDallete =(ider)=>{
       ...
       const proceed = window.confirm('Are you sure?');
           ...
           const url = `https://deltaserverer.onrender.com/company3/${id}`;
           console.log(url);
           fetch(url, {
               method: 'DELETE',
               headers: {
